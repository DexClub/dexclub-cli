...
def fail_missing_sources() -> None:
    ...
    print(
        ...
        "The skill itself can still run with the pre-generated references in analyst/references/query-json.md "
        "and analyst/references/query-json.schema.json.",
        file=sys.stderr,
    )
    raise SystemExit(1)


def resolve_source_dirs(skill_root: Path) -> tuple[Path, Path]:
    ...
    resolved = validate_source_dirs(skill_root.parent.parent / DEXKIT_SOURCE_RELATIVE_DIR)
    if resolved is not None:
        return resolved
    fail_missing_sources()
...
def render_markdown(query_defs: dict[str, TypeDef], result_defs: dict[str, TypeDef]) -> str:
    ...
    lines.append("")
    lines.append("> Generated by `analyst/scripts/generate_query_reference.py`. Do not hand-edit this file.")
    ...
    lines.append("## Schema")
    lines.append("")
    lines.append("- Machine-readable schema: `analyst/references/query-json.schema.json`")
    lines.append("")
    return "\n".join(lines)
...
def main() -> None:
    ...
    analyst_dir = script_dir.parent
    skill_root = analyst_dir.parent
    query_dir, result_dir = resolve_source_dirs(skill_root)
    references_dir = analyst_dir / "references"
